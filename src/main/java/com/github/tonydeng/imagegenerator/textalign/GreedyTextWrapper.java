...
public class GreedyTextWrapper implements TextWrapper {
    ...
    public List<String> doWrap(String text, int lineWidth, FontMetrics fm) {
        ...
        final List<String> lines = judgeText(replaceText(text), lineWidth, fm);
        final StringBuilder builder = new StringBuilder();

        int spaceLeft = lineWidth;

        boolean removed = false, nospaceLeft;
        String word = "";
...
//     */
//    @Deprecated
//    public List<String> doWrapByA(String text, int lineWidth, FontMetrics fm) {
...
//            word.getChars(0, word.length(), chars, 0);
//            for (int i = 0; i < chars.length; i++) {
//                log.info("i:'{}' char:'{}' build:'{}' spaceLeft:'{}' charWidth:'{}'", i, chars[i], builder, spaceLeft, fm.charWidth(chars[i]));
//                if (fm.charWidth(chars[i]) > spaceLeft) {
//                    if (chars[i] != CSPACE) {
//                        builder.delete(builder.length() - i, builder.length());
//                        removed = true;
